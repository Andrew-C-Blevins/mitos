...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /people/{id} {
      ...
      // Household roles, unique colors and the final-admin guard are enforced in the authenticated people adapter.
    ...
    match /items/{id} {
      ...
      match /log/{entryId} {
        ...
        allow create: if access(getAfter(/databases/$(database)/documents/items/$(id)).data)
          ...
          && request.resource.data.text is string && request.resource.data.text.size() > 0 && request.resource.data.text.size() <= (request.resource.data.kind == 'capture' ? 50000 : 4000)
